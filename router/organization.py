...
async def auto_login(user:dict):
    ...
    access_token = create_access_token(
            data={"sub": user["user_email"],"role":user["role"]}, expires_delta=access_token_expires
        )
    return {"access_token": access_token,"token_type": "bearer","user_id":user["user_id"],"user_name":user["user_name"],'role':user["role"],"org_id":user["org_id"]}
...
async def create_organization(org: OrganizationBase, db: db_dependency):
    try:
        ...
        org_instance.password = org.password  # You should hash the password here
        db.add(org_instance)
        db.commit()
        ...
        payload = {
            ...
            "org_name":org_instance.org_name
        }
        return {"detail": "Organization created successfully", "org_id": org_instance.org_id,"payload":payload}
